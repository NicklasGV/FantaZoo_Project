...
public class CageController {
    ...
    @PutMapping("/id/{id}")
    Cage updateCage(@RequestBody Cage updatedcage, @PathVariable int id) {
        return cr.findById(id)
          .map(cage -> {
            cage.setName(updatedcage.getName());
            cage.setZookeepers(updatedcage.getZookeepers());
            cage.setAnimals(updatedcage.getAnimals());
            return cr.save(cage);
          })
          .orElseGet(() -> {
            updatedcage.setId(id);
            return cr.save(updatedcage);
          });
    }
